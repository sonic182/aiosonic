...
class SSEConnection:
    ...
    def __init__(self, response, client: HTTPClient, url: str, headers: Dict[str, str], reconnect: bool, retry_delay: int):
    ...
    async def __aiter__(self) -> AsyncIterator[SSEEvent]:
        ...
        while not self._closed:
            try:
                ...
                self._response = await self._client.get(self._url, headers=self._headers)
...
class _ConnectAwaitable:
    ...
    def __init__(self, client: HTTPClient, url: str, headers: Dict[str, str], reconnect: bool, retry_delay: int):
        self._client = client
        self._url = url
        self._headers = headers
        self._reconnect = reconnect
    ...
    async def _do_connect(self) -> SSEConnection:
        while True:
            try:
                ...
                conn = SSEConnection(response, self._client, self._url, dict(self._headers), self._reconnect, self._retry_delay)
                if not self._reconnect:
                    try:
